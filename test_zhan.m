...
names = {'Arctic'; 'R(I)'; 'R(II)'; 'R(III)'; 'R(IV)'; 'R(V)'};
figure('Renderer', 'painters', 'Position', [10 10 900 600])
for month=1:6  
    subplot(3,2,month);
    b = bar(squeeze(var(:,:,month)),'EdgeColor','k','BarWidth',1);
    b(1).FaceColor = '#919191';
    b(2).FaceColor = '#869BF8';
    b(3).FaceColor = '#F1948F';
    axis(axes(month,:))
    set(gca,'xtick',1:6,'xticklabel',names)
    set(gca,'FontSize',20)
    if month ==1
        legend([b(1),b(2),b(3)],{'total ASR','sfc.','atm.'},'FontSize',20)
    end
end
