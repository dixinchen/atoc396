...
toc;
%%
var_nn = zeros(2,4,size(lon_nn,1),idx_lat1_nn-idx_lat2_nn+1);
name = ["dRa","dRc","dRw","dR"];
for toa_sfc = 1:2
    for variables = 1:4
...
toc;
%%
% ww = squeeze(var_nn(1,3,:,:));

%%
var_k = zeros(2,4,size(lon_k,1),idx_lat1_k-idx_lat2_k+1);
name = ["dRa","dRc","dRw","dR"];
for toa_sfc = 1:2
    for variables = 1:4
...
%%
var_true = zeros(2,size(lon_nn,1),idx_lat1_nn-idx_lat2_nn+1);
for toa_sfc = 1:2
    if toa_sfc == 1
    ...
    x = squeeze(data(:,idx_lat2_nn:idx_lat1_nn, month, :));
    ita_x = zeros(size(x));
    for i=1:size(x,1)
        for j = 1:size(x,2)
            a = squeeze(x(i,j,:));
    ...
    var_true(toa_sfc,:,:) = nanvar(ita_x,0,3);
end
toc;
...
struct_tmp = struct('name','var_toa_dr','type','var','nc_type','NC_FLOAT','var_name',[]);
struct_tmp.dim = {'longitude';'latitude'};
struct_tmp.att = struct('short_name','Variance SFC NN dRc',...
    'units','(W/m^2)^2','missing_values',-9999);
[status7] = netcdf_write(filename,squeeze(var_nn(1,4,:,:)),struct_tmp);

struct_tmp = struct('name','var_sfc_dr','type','var','nc_type','NC_FLOAT','var_name',[]);
struct_tmp.dim = {'longitude';'latitude'};
struct_tmp.att = struct('short_name','Variance SFC NN dRw',...
    'units','(W/m^2)^2','missing_values',-9999);
[status8] = netcdf_write(filename,squeeze(var_nn(2,4,:,:)),struct_tmp);

struct_tmp = struct('name','var_toa_truth','type','var','nc_type','NC_FLOAT','var_name',[]);
struct_tmp.dim = {'longitude';'latitude'};
struct_tmp.att = struct('short_name','Variance SFC NN dRw',...
    'units','(W/m^2)^2','missing_values',-9999);
[status9] = netcdf_write(filename,squeeze(var_true(1,:,:)),struct_tmp);

struct_tmp = struct('name','var_sfc_truth','type','var','nc_type','NC_FLOAT','var_name',[]);
struct_tmp.dim = {'longitude';'latitude'};
struct_tmp.att = struct('short_name','Variance SFC NN dRw',...
    'units','(W/m^2)^2','missing_values',-9999);
[status9] = netcdf_write(filename,squeeze(var_true(2,:,:)),struct_tmp);
...
struct_tmp = struct('name','latitude','type','dim','nc_type','NC_FLOAT');
netcdf_write(filename,lat_k(idx_lat2_k:idx_lat1_k),struct_tmp);
%save data
struct_tmp = struct('name','var_toa_dra','type','var','nc_type','NC_FLOAT','var_name',[]);
struct_tmp.dim = {'longitude';'latitude'};
...
struct_tmp.att = struct('short_name','Variance SFC kernel dRc',...
    'units','(W/m^2)^2','missing_values',-9999);
[status7] = netcdf_write(filename,squeeze(var_k(1,4,:,:)),struct_tmp);

struct_tmp = struct('name','var_sfc_dr','type','var','nc_type','NC_FLOAT','var_name',[]);
struct_tmp.dim = {'longitude';'latitude'};
struct_tmp.att = struct('short_name','Variance SFC kernel dRw',...
    'units','(W/m^2)^2','missing_values',-9999);
[status8] = netcdf_write(filename,squeeze(var_k(1,4,:,:)),struct_tmp);