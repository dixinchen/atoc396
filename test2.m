...
%     dfal_dt = 2*(dfal_dt-repmat(min(dfal_dt),[1 num]))./repmat(max(dfal_dt)-min(dfal_dt),[1 num])-1;
    ...
    fal = fal(idx_lon_nn, idx_lat_nn, 12*28+i:12:12*37+i);
    fal = reshape(fal,[1 1 num]).*repmat(W,[1 1 num]);
...
%     dR_m(i,:) = dR_nn;
    ...
    clim = ncread([gen_path,'data_1_m/Test2007_2016/fal_mean_10.nc'],'fal_mean');
    clim_m(i,1) = squeeze(clim(idx_lon_nn, idx_lat_nn, i));
end

% month = 9;
% clim_m = ncread([gen_path,'data_1_m/Test2007_2016/fal_mean_10.nc'],'fal_mean');
...
ylabel('dRa [W/m^2]','FontSize',16);xlabel('fal','FontSize',16);title('2007-2016 NN estimation','FontSize',16)
